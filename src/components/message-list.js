...
class MessageList extends Component {
    ...
    hndViewChange(ev){
        ...
        if(ev.target!==undefined){
            detId = ev.target.parentElement.dataset.content-2;
        }
    ...
    render() {let content;
        if(this.state.navForm==="list"){
            ...
            content = <MessageDetails msgDetails={this.state.msgList[this.state.detailsId]} 
                backNav={this.hndViewChange.bind(this)}
                hndUpdate={this.getMessageList.bind(this)}/>;            
        }
        return (
            <Container fluid className={"dashboard-form align-items-start p-0 py-5 bg-white d-flex minh-75vh " + this.props.classExt}>
                <Row className="mx-auto text-center w-100">
                    ...
                    <Col xs={12} md={9} className="mx-auto p-0 d-flex align-items-start p-3">
                        <Card className="w-100 minh-50vh">
                            <Card.Body>
                                <p className="initialism text-secondary text-left border-bottom border-secondary mb-4">
                                    Messages
                                </p>
                                {content}
                            </Card.Body>
